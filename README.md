...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`two-face`](https://crates.io/crates/two-face).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `two-face` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Extra TextMate themes for syntect
...
Original work copyrighted by the upstream `two-face` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
