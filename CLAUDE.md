# Claude Code Project Instructions — two-face-kotlin

## Project Overview

This is **two-face-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`two-face`](https://crates.io/crates/two-face).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/two-face/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/two-face>
